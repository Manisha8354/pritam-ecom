...
export default function Orders() {
  ...
  async function getProfile() {
    try {
      let result = await axios.get('https://actl.co.in/suchit/getOrder');
      if(result){
        setData(result.data);
      }
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    ...
    if(flag){
     await axios.delete(`https://actl.co.in/suchit/deleteProduct/${id}`);
     getProfile()
    }
  };
  ...
  async function handleUpdate(data, id) {
    ...
    if(flag){
    await axios.put(`https://actl.co.in/suchit/updateOrder/${id}`, {
        'status':data
    });
    getProfile()
    }
